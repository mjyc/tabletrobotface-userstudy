...
export function extractFaceFeatures(poses) {
  if (poses.length === 0 || (
    !poses[0].keypoints.find(function (kpt) {return kpt.part === 'nose';})
  ...
  var v = {  // a vector from the point between two eyes to the nose
    x: ns.x - btnEyesPt.x,
    y: ns.y - btnEyesPt.y,
  };
  var faceOrientation = Math.atan2(v.y, v.x);

  var dbere = Math.sqrt(Math.pow(btnEyesPt.x - re.x, 2) + Math.pow(btnEyesPt.y - re.y, 2));
  var dbens = Math.sqrt(Math.pow(ns.x - btnEyesPt.x, 2) + Math.pow(ns.y - btnEyesPt.y, 2));
  var noseOrientation = Math.acos((Math.pow(dbere, 2) + Math.pow(dbens, 2) - Math.pow(dnsre, 2)) / (2 * dbere * dbens));
