...
function extractFaceFeatures(poses) {
  ...
  var dbwre = Math.sqrt(Math.pow(bw.x - re.x, 2) + Math.pow(bw.y - re.y, 2));
  var dbwns = Math.sqrt(Math.pow(ns.x - bw.x, 2) + Math.pow(ns.y - bw.y, 2));
  var noseRotation = Math.acos((Math.pow(dbwre, 2) + Math.pow(dbwns, 2) - Math.pow(dnsre, 2)) / (2 * dbwre * dbwns));
